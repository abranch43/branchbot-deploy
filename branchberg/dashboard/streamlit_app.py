...
st.set_page_config(page_title="BranchOS Income Ingest", page_icon="💰", layout="wide")

# Sidebar navigation
st.sidebar.title("BranchOS Dashboard")
page = st.sidebar.radio(
    "Navigation", ["Income Ingest", "Revenue Summary", "Transaction History"]
)
...
if page == "Income Ingest":
    ...
    st.markdown(
        "Track all revenue streams in one place - manual entries, CSV imports, and automated webhooks."
    )
    ...
    if summary:
        ...
        with col1:
            st.metric("Total Revenue", f"${summary['total_dollars']:,.2f}", delta=None)
        with col2:
            st.metric("Transaction Count", f"{summary['count']:,}", delta=None)
    ...
    with col1:
        ...
        with st.form("manual_entry"):
            amount = st.number_input(
                "Amount ($)", min_value=0.01, value=100.00, step=0.01
            )
            ...
            if submitted:
                ...
                if result:
                    st.success(
                        f"✅ Transaction added successfully! ID: {result['event_id']}"
                    )
                    st.rerun()
                else:
                    st.error(error)
    ...
    with col2:
        ...
        st.markdown(
            "Upload a CSV file with revenue transactions. Map your columns below."
        )
        ...
        if uploaded_file:
            ...
            currency_col = st.selectbox(
                "Currency Column (optional)", [""] + columns, key="currency_col"
            )
            email_col = st.selectbox(
                "Email Column (optional)", [""] + columns, key="email_col"
            )
            entity_col = st.selectbox(
                "Entity Column (optional)", [""] + columns, key="entity_col"
            )
            description_col = st.selectbox(
                "Description Column (optional)", [""] + columns, key="desc_col"
            )
            ...
            if st.button("Upload & Process CSV"):
                ...
                if result:
                    st.success(
                        f"✅ CSV processed! Created {result['created_count']} transactions out of {result['total_rows']} rows."
                    )
    ...
    if summary:
        ...
        with col3:
            avg = (
                summary["total_dollars"] / summary["count"]
                if summary["count"] > 0
                else 0
            )
            st.metric(
                "Average Transaction", f"${avg:,.2f}", help="Average transaction value"
            )
    ...
    if events:
        ...
        st.info(
            "No transactions found. Add some transactions using the Income Ingest page!"
        )
