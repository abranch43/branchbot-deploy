...
def get_test_db_engine():
    ...
    test_db_path = os.path.join(
        tempfile.gettempdir(), f"test_branchbot_{uuid.uuid4().hex[:8]}.db"
    )
...
def test_revenue_events():
    ...
    # Add transactions
    client.post(
        "/ingest/manual", json={"amount": 100.00, "customer_email": "user1@test.com"}
    )
    client.post(
        "/ingest/manual", json={"amount": 200.00, "customer_email": "user2@test.com"}
    )
